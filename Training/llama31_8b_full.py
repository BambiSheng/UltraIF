...
def sharegpt_map_fn(example):
    ...
    if "history" in example:
        ...
        for i in range(0, len(history), 2):
            single_turn_conversation = {"input": history[i]["value"], "output": history[i+1]["value"], "output_with_loss": False}
            conversation.append(single_turn_conversation)
    conversation.append({"input": example["instruction"], "output": example["output"], "output_with_loss": True})
    
    # conversation = [{"input": example["instruction"], "output": example["output"]}]
...
train_dataset = dict(
    ...
    dataset_map_fn=sharegpt_map_fn,
